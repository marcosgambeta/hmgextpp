...
using ARGB = ULONG;
using Func_GdiPlusStartup = GpStatus(WINGDIPAPI *)(ULONG_PTR *, GDIPCONST GdiplusStartupInput *,
...
//*************************************************************************************************************
//  bt_LoadGDIPlusPicture(FileName, TypePicture) ---> Return hBitmap  (Load BMP, GIF, JPG, TIF and
//  PNG image)
//*************************************************************************************************************
...
//***********************************************************************************************************************
// bt_SaveGDIPlusPicture(hBitmap, FileName, TypePicture) // Return TRUE/FALSE  (Save BMP, GIF, JPG,
// TIF and PNG image)
//***********************************************************************************************************************
...
// bt_SaveGDIPlusPicture(hBitmap, FileName, TypePicture) ---> Return TRUE/FALSE  (Save BMP, GIF,
// JPG, TIF and PNG image)
...
HB_FUNC(BT_SCR_INVALIDATERECT)
{
  ...
  if (hb_arrayLen(pArrayRect) == 4)
  {
    ...
    hb_retl(InvalidateRect(hmg_par_HWND(1), &rect,
                           hb_parl(3))); // Invalidate specific rectangle of client area
  }
  else
  {
    hb_retl(false);
...
/*
BT_DRAW_HDC_ARCX(HDC, x1, y1, x2, y3, XStartArc, YStartArc, xEndArc, YEndArc, colorLine, widthLine,
colorFill, arcType) --> NIL
*/
HB_FUNC(BT_DRAW_HDC_ARCX)
{
  auto hDC = hmg_par_HDC(1);
  auto x1 = hmg_par_INT(2);
  auto y1 = hmg_par_INT(3);
  auto x2 = hmg_par_INT(4);
  auto y2 = hmg_par_INT(5);

  auto XStartArc = hmg_par_INT(6);
  auto YStartArc = hmg_par_INT(7);
...
/*
BT_DRAW_HDC_FILLEDOBJECT(HDC, x, y, width, height, colorFill, colorLine, widthLine, type,
roundWidth, roundHeight) --> NIL
*/
...
/*
BT_DRAW_HDC_BITMAP(HDC, x1, y1, width1, height1, HBITMAP, x2, y2, width2, height2, modeStretch,
action, colorTransp) --> .T.|.F.
*/
HB_FUNC(BT_DRAW_HDC_BITMAP)
{
  ...
  auto x2 = hmg_par_INT(7);
  auto y2 = hmg_par_INT(8);
...
/*
BT_DRAW_HDC_BITMAPALPHABLEND(HDC, x1, y1, width1, height1, HBITMAP, x2, y2, width2, height2, alpha,
modeStretch) --> NIL
*/
...
/*
BT_DRAW_HDC_TEXTOUT(HDC, x, y, text, fontname, fontsize, textColor, backColor, type, align,
orientation) --> NIL
*/
...
/*
BT_DRAW_HDC_DRAWTEXT(HDC, x, y, w, h, text, fontName, fontSize, textColor, backColor, type, align,
orientation) --> NIL
*/
HB_FUNC(BT_DRAW_HDC_DRAWTEXT)
{
  ...
  auto x = hmg_par_INT(2);
  auto y = hmg_par_INT(3);
  auto w = hmg_par_INT(4);
  auto h = hmg_par_INT(5);
#ifndef UNICODE
...
/*
BT_DRAW_HDC_TO_HDC(HDC1, x1, y1, width1, height1, HDC2, x2, y2, width2, height2, modeStretch,
action, colorTransp) --> .T.|.F.
*/
...
/*
BT_DRAW_HDC_TO_HDC_ALPHABLEND(HDC1, x1, y1, width1, height1, HDC2, x2, y2, width2, height2, alpha,
modeStretch) --> NIL
*/
...
HB_FUNC(BT_BMP_LOADFILE)
{
  ...
  //       hBitmapFile = bt_bmp_convert_to_24bpp(hBitmap, BMP_DELETE_ORIGINAL_HBITMAP);   // Convert
  //       Bitmap in 24bpp
...
//*********************************************************************************************************************************
//* BT_BitmapLoadEMF(cFileName, [aRGBBackgroundColor], [nNewWidth], [nNewHeight], [ModeStretch])
//---> Return hBITMAP
//*********************************************************************************************************************************
  ...
  for (auto Row = 0; Row < Image2.Height; Row++)
  {
    for (auto Col = 0; Col < Image2.Width; Col++)
    {
      ...
      { // color channel C = R,G,B
        ...
        Color = a * (1.00 - x_diff) * (1.00 - y_diff) + b * (x_diff) * (1.00 - y_diff) +
                c * (y_diff) * (1.00 - x_diff) + d * (x_diff * y_diff);
...
HB_FUNC(BT_BMP_COPYANDRESIZE)
{
  ...
  auto New_Width = hmg_par_INT(2);
  auto New_Height = hmg_par_INT(3);
  auto Mode_Stretch = hmg_par_INT(4);
...
/*
BT_BMP_PASTE(HBITMAP1, x1, y1, width1, height1, HBITMAP2, x2, y2, width2, height2, stretch, action)
--> .T.|.F.
*/
...
/*
BT_BMP_PASTE_ALPHABLEND(HBITMAP1, x1, y1, width1, height1, HBITMAP2, x2, y2, width2, height2) -->
NIL
*/
      ...
      if ((y >= HALF && y < (bm.bmHeight - HALF)) && (x >= HALF && x < (bm.bmWidth - HALF)))
      {
        ...
        /*
           ...
           #define BT_FILTER_FULL   255
                      INT Alpha = 200;  // transparent = color origin = 0 To 255 = opaque = full
           filter color RGBcolor_D->R = (BYTE)((RGBcolor_Ret.R * Alpha + RGBcolor_O->R * (255 -
           Alpha)) / 255); RGBcolor_D->G = (BYTE)((RGBcolor_Ret.G * Alpha + RGBcolor_O->G * (255 -
           Alpha)) / 255); RGBcolor_D->B = (BYTE)((RGBcolor_Ret.B * Alpha + RGBcolor_O->B * (255 -
           Alpha)) / 255);
         */
      }
      //         RGBcolor_O ++;
...
{
  auto Width1 = hmg_par_INT(1);
  auto Height1 = hmg_par_INT(2);
  auto Width2 = hmg_par_INT(3);
  auto Height2 = hmg_par_INT(4);
  auto Mode_Stretch = hb_parnl(5);
